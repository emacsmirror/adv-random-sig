...
;; $Id: adv-random-sig.el,v 1.2 2000/10/28 15:08:30 rscholz Exp $
...
subdirectory or file in `adv-signature-dir'."
  ...
  (with-temp-buffer
	...
	(if (file-readable-p sig-dir-file)
	  ...
	  ;; none of that
	  (insert-string (concat "*** Unable to find quote file or directory '"
				 sig-dir-file "' ***\n")))))
...
(defun adv-signature-greeting-string ()
  ...
  (with-temp-buffer
	...
	(insert-file-contents adv-signature-greeting)
      (insert-string (concat "*** Unable to insert greeting file '"
...
subdirectory or file in function `adv-signature-quote-string'."
  ...
  (with-temp-buffer
	...
	(insert-string (adv-signature-head-string)))
    ...
    (insert-string (adv-signature-quote-string flavour))
...
subdirectory or file in function `adv-signature-quote-string'."
  ...
  (with-temp-buffer
	...
	(insert-string (adv-signature-greeting-string)))
    ...
    (insert-string (adv-random-signature-no-sep flavour))
...
subdirectory or file in function `adv-signature-quote-string'."
  ...
  (insert-string (adv-random-signature flavour)))
